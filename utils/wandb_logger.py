        ...
        for a in self.agents:
            if self.agent_sleep_rates[a]:
                summary[a] = {
                    ...
                    'avg_entropy': np.mean(self.agent_entropies[a][-self.window_size:]) if self.agent_entropies[a] else 0.0
                }
...
def log_episode_timeline(env, actors, entropy_threshold, max_sleep_ticks, device, hidden_size=64, episode='Eval'):
    ...
    timeline_data = {
        agent: {'tick_rate': [], 'entropy': [], 'action': [], 'reward': []} for agent in agents
    }
    
    max_frames = 150
    
    hidden_states = {
        a: torch.zeros((1, hidden_size)).to(device) for a in agents
    }
    ...
    with torch.no_grad():
        for frame in range(max_frames):
            ...
            next_obs_dict, next_global_state, rewards_dict, dones_dict, _, active_masks = env.step(actions_dict, sleep_ticks_dict)
...
def generate_money_shot_figure(timeline_data, agents, save_path=None, title='ETD-MAPPO Money Shot: Entropy vs Tick Rate'):
    """
...
def log_temporal_specialization(tracker, update_num):
    ...
    wandb.log({
        f'temporal_specialization/sleep_rates_update_{update_num}': wandb.plot.bar(
            table, 'Agent', 'Sleep Rate (%)', 
            title=f'Per-Agent Sleep Rates (Update {update_num})'
        ),
        f'temporal_specialization/entropies_update_{update_num}': wandb.plot.bar(
            table, 'Agent', 'Avg Entropy',
            title=f'Per-Agent Average Entropy (Update {update_num})'
        )
    })
    ...
    for a in agents:
        print(f"    {a}: Sleep Rate = {summary[a]['avg_sleep_rate']:.1f}%, Avg Entropy = {summary[a]['avg_entropy']:.3f}")
...
def log_comparison_table(summary, update_num):
    ...
    table = wandb.Table(
        data=data,
        columns=['Method', 'Reward (mean ± std)', 'Win Rate', 'FLOP Reduction']
    )
    ...
    print(f"    {'-'*60}")
    for mode in ['vanilla', 'fixed_skip', 'etd']:
        print(f"    {mode.upper():<12} {summary[mode]['mean_reward']:>6.2f} ± {summary[mode]['std_reward']:<6.2f}   "
